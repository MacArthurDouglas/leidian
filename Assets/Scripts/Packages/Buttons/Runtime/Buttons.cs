...
namespace com.yihui.Buttons
{
    ...
    public class Buttons : MonoBehaviour
    {
        ...
        [Header("启用按钮遮挡检测")]
        [SerializeField]
        private bool enableBlockCheck = true; //启用按钮的遮挡检测，按钮被其他角色遮挡后会失效


        [Header("是否开启默认特效")]
        [SerializeField]
        private bool enableDefaultSpecialEffect=true;  //是否开启默认特效
        ...
        //按钮是否可交互
        public bool Interactable=> activated &&IsLogicEnabled()&& !onCooling;
        ...
        public void SwitchToBrightMaterial()
        {
            spriteRenderer.material=Resources.Load<Material>("Material/BrightMaterial"); 
            spriteMaterial = GetComponent<SpriteRenderer>().material;
            brightnessId=Shader.PropertyToID("_Brightness");
        }
        
        /**
         * 需要搭配BrightSprite.shader使用
         */
        public void DefaultBrightOn(float brightness)
        {
            spriteMaterial.SetFloat(brightnessId, brightness);
        }
        public void DefaultBrightOff()
        {
            spriteMaterial.SetFloat(brightnessId, 1f);
        }
    }
}